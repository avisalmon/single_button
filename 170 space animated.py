...
def animate_timing():
    ...
    global game_on
    while game_on:
        sleep_ms(animating_time)
        animate_tick = 1

th.start_new_thread(animate_timing, ())


class Invador(Sprite):
    ...
    def check_collide(self, x, y, ext_size):
        if ((self.x_pos < x < self.x_pos + self.size[0]) and (self.y_pos < y < self.y_pos + self.size[1])) or ((self.x_pos < x+ext_size < self.x_pos + self.size[0]) and (self.y_pos < y+ext_size < self.y_pos + self.size[1])):
            return True
        else:
            return False
...
for j in range(3):
    for k in range(4):
        invadors.append(Invador(start_x + k*space, start_y + j*int(space/1.2), image=['space_1_0.bin', 'space_1_1.bin'], size=(16,16), pattern='imag'))
...
while game_on:
    ...
    for invador in invadors:
        if invador.y_pos + y_offset > 45:
            game_on = False
        invador.draw(display, offset = y_offset)
